...
export function AdminDashboard({ ownerName }: { ownerName: string }) {
  ...
  return <main className="min-h-screen bg-[#fff7eb] text-[#2e201c]"><header className="border-b bg-white"><div className="mx-auto flex max-w-[1500px] items-center justify-between px-4 py-4"><div className="flex items-center gap-3"><div className="grid size-11 place-items-center rounded-2xl bg-[#a82d1e] text-white"><Store /></div><div><h1 className="font-black">MELI · Quản lý đơn</h1><p className="text-xs text-zinc-500">Xin chào, {ownerName}</p></div></div><div className="flex gap-2"><Button variant="outline" size="icon" onClick={() => load()} aria-label="Làm mới"><RefreshCw /></Button><Link href="/"><Button variant="outline">Menu khách</Button></Link><Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button className="bg-[#bd3b22] hover:bg-[#9e281c]"><Plus /> Tạo đơn</Button></DialogTrigger><CreateOrderDialog onCreated={() => { setOpen(false); load(true); }} /></Dialog></div></div></header>
    <section className="mx-auto max-w-[1500px] px-4 py-6"><div className="mb-6 grid gap-3 sm:grid-cols-3"><Stat label="Đơn đang mở" value={`${activeCount}`} /><Stat label="Đã thanh toán" value={`${orders.filter((o) => o.status === "paid").length} đơn`} /><Stat label="Doanh thu ghi nhận" value={formatMoney(todayRevenue)} /></div>{error && <p className="mb-5 rounded-2xl bg-red-50 p-4 text-red-700">{error}</p>}<div className="grid gap-4 xl:grid-cols-4">{columns.map((column) => { const Icon = column.icon; const list = orders.filter((order) => order.status === column.id); return <div key={column.id} className="min-h-64 rounded-3xl bg-[#eaede8] p-3"><div className="mb-3 flex items-center justify-between px-2"><h2 className="flex items-center gap-2 font-black"><Icon className="size-4" />{column.title}</h2><span className="rounded-full bg-white px-2.5 py-1 text-xs font-black">{list.length}</span></div><div className="space-y-3">{loading && !orders.length ? <div className="rounded-2xl bg-white p-5 text-sm text-zinc-500">Đang tải...</div> : list.map((order) => <article key={order.id} className="rounded-2xl bg-white p-4 shadow-sm"><div className="flex justify-between gap-2"><div><p className="text-xs font-bold text-[#9e281c]">{order.code} · {order.source === "staff_pos" ? "Chủ quán tạo" : "Khách tự đặt"}</p><h3 className="text-lg font-black">{order.tableCode ? `Bàn ${order.tableCode}` : "Mang về"}</h3></div><time className="text-xs text-zinc-500">{new Date(order.createdAt + "Z").toLocaleTimeString("vi-VN", { hour: "2-digit", minute: "2-digit" })}</time></div><div className="my-3 space-y-1 border-y py-3">{order.items.map((item) => <div key={item.id} className="flex justify-between text-sm"><span><b>{item.quantity}×</b> {item.itemName}</span><span>{formatMoney(item.price * item.quantity)}</span></div>)}</div>{order.note && <p className="mb-3 rounded-xl bg-[#fff0df] p-2 text-xs"><b>Ghi chú:</b> {order.note}</p>}<div className="flex items-center justify-between"><b>{formatMoney(order.total)}</b>{column.next && <Button size="sm" className="bg-[#a82d1e]" onClick={() => updateStatus(order.id, column.next)}>{column.action}</Button>}</div></article>)}</div></div>; })}</div></section>
  </main>;
}
